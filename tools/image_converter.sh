...
FN_Check() {
    if ! command -v $TOOL &> /dev/null; then
        FN_Banner "$TOOL is not supported. Please install by the following commands:"
        FN_Banner "sudo add-apt-repository ppa:ondrej/php"
        FN_Banner "sudo apt-get update"
        FN_Banner "sudo apt-get install php7.2-cli"
        exit 1
    fi
}
